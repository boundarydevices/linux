...
static void bch_data_invalidate(struct closure *cl)
{
	...
	while (bio_sectors(bio)) {
		...
		bch_keylist_add(&s->insert_keys,
				&KEY(s->inode, bio->bi_iter.bi_sector, len));
	}
...
static void cached_dev_read_done(struct closure *cl)
{
	...
	if (s->cache_bio) {
		...
		s->cache_bio->bi_iter.bi_sector =
			s->cache_miss->bi_iter.bi_sector;
		s->cache_bio->bi_bdev = s->cache_miss->bi_bdev;
		s->cache_bio->bi_iter.bi_size = s->cache_bio_sectors << 9;
				 ...
				 struct bio *bio, unsigned sectors)
{
	...
	s->replace_key = KEY(s->inode, bio->bi_iter.bi_sector +
	...
	cache_bio->bi_iter.bi_size	= s->cache_bio_sectors << 9;
