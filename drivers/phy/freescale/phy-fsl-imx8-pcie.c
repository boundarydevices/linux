...
#define IMX8MP_PCIE_PHY_FLAG_CLK_MASK	GENMASK(2, 0)
#define IMX8MP_PCIE_PHY_FLAG_XO_PLL	BIT(0)
#define IMX8MP_PCIE_PHY_FLAG_EXT_OSC	BIT(1)
#define IMX8MP_PCIE_PHY_FLAG_SYS_PLL	BIT(2)
...
static int imx8_pcie_phy_init(struct phy *phy)
{
	...
	if (imx8_phy->flags & IMX8MP_PCIE_PHY_FLAG_SYS_PLL) {
...
static int imx8_pcie_phy_probe(struct platform_device *pdev)
{
	...
	imx8_phy->flags &= ~IMX8MP_PCIE_PHY_FLAG_CLK_MASK;
	if (of_property_read_u32(np, "clk_mode", &val) < 0)
		/* Use the external OSC as defaul CLK mode */
		imx8_phy->flags |= IMX8MP_PCIE_PHY_FLAG_EXT_OSC;
	dev_info(dev, "clk mode %d.\n", val);
	if (val == 1)
		imx8_phy->flags |= IMX8MP_PCIE_PHY_FLAG_XO_PLL;
	else if (val == 2)
		imx8_phy->flags |= IMX8MP_PCIE_PHY_FLAG_EXT_OSC;
	else if (val == 3)
		imx8_phy->flags |= IMX8MP_PCIE_PHY_FLAG_SYS_PLL;
