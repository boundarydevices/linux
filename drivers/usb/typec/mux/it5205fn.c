...
static int it5205fn_switch_set(struct typec_switch_dev *sw,
...
static int it5205fn_mux_set(struct typec_mux_dev *mux, struct typec_mux_state *state)
{
...
)
{
	...
	dev_dbg(dev, "%s job finished.\n", __func__);
	return ret;
}

static void it5205fn_remove(struct i2c_client *client)
{
