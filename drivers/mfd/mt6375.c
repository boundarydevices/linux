...
static int mt6375_irq_map(struct irq_domain *d, unsigned int virq,
			  irq_hw_number_t hwirq)
{
	struct mt6375_data *ddata = d->host_data;
...
static irqreturn_t mt6375_irq_thread(int irq, void *data)
{
	...
	ret = regmap_bulk_write(ddata->rmap, MT6375_REG_CHG_IRQ0 + start,
				evt + start, evt_count);
	if (ret)
...
static int mt6375_probe(struct i2c_client *client)
{
	...
	for (i = 0; i < MT6375_SLAVE_MAX; i++) {
		...
		if (IS_ERR(ddata->i2c[i])) {
			dev_err(ddata->dev, "failed to new i2c(0x%02X)\n",
				mt6375_slave_addr[i]);
...
{
	struct i2c_client *i2c = to_i2c_client(dev);
	struct mt6375_data *ddata = i2c_get_clientdata(i2c);

	atomic_set(&ddata->in_sleep, 1);
	return 0;
}
...
static const struct dev_pm_ops mt6375_pm_ops = {
	...
	SET_NOIRQ_SYSTEM_SLEEP_PM_OPS(mt6375_suspend_noirq, mt6375_resume_noirq)
};
