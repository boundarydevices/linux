...
#define RPMSG_RAW_DEVNAME "rpmsg-raw"
#define RPMSG_RAW_MAX	(MINORMASK + 1)
...
int rpmsg_raw_eptdev_destroy(struct device *dev, void *data)
{
...
EXPORT_SYMBOL(rpmsg_raw_eptdev_destroy);
			       ...
			       unsigned long arg)
{
	...
	return rpmsg_raw_eptdev_destroy(&eptdev->dev, NULL);
}
...
static struct rpmsg_eptdev *__rpmsg_raw_eptdev_create(struct rpmsg_device *rpdev,
							 ...
							 struct rpmsg_channel_info chinfo)
{
	...
	ret = ida_simple_get(&rpmsg_minor_ida, 0, RPMSG_RAW_MAX, GFP_KERNEL);
	if (ret < 0)
		goto free_eptdev;
...
int rpmsg_raw_eptdev_create(struct rpmsg_device *rpdev, struct device *parent,
			       struct rpmsg_channel_info chinfo)
{
	struct rpmsg_eptdev *eptdev;

	eptdev = __rpmsg_raw_eptdev_create(rpdev, parent, chinfo);
	if (IS_ERR(eptdev))
		return PTR_ERR(eptdev);

	return 0;
}
EXPORT_SYMBOL(rpmsg_raw_eptdev_create);
...
static int rpmsg_raw_probe(struct rpmsg_device *rpdev)
{
	...
	memcpy(chinfo.name, RPMSG_RAW_DEVNAME, sizeof(RPMSG_RAW_DEVNAME));
	chinfo.src = rpdev->src;
	chinfo.dst = rpdev->dst;

	eptdev =  __rpmsg_raw_eptdev_create(rpdev, &rpdev->dev, chinfo);
	if (IS_ERR(eptdev))
		return PTR_ERR(eptdev);
...
static void rpmsg_raw_remove(struct rpmsg_device *rpdev)
{
	...
	ret = device_for_each_child(&rpdev->dev, NULL, rpmsg_raw_eptdev_destroy);
	if (ret)
		dev_warn(&rpdev->dev, "failed to destroy endpoints: %d\n", ret);
}

static struct rpmsg_device_id rpmsg_raw_id_table[] = {
	{ .name	= RPMSG_RAW_DEVNAME },
	{ },
};

static struct rpmsg_driver rpmsg_raw_driver = {
	.probe = rpmsg_raw_probe,
	.remove = rpmsg_raw_remove,
	.id_table = rpmsg_raw_id_table,
	.drv.name = "rpmsg_raw",
};
...
static int rpmsg_raw_init(void)
{
	...
	ret = alloc_chrdev_region(&rpmsg_major, 0, RPMSG_RAW_MAX, "rpmsg_raw");
	if (ret < 0) {
		pr_err("rpmsg: failed to allocate char dev region\n");
		return ret;
	}

	rpmsg_class = class_create(THIS_MODULE, "rpmsg_raw");
	...
	ret = register_rpmsg_driver(&rpmsg_raw_driver);
	if (ret < 0) {
...
free_region:
	unregister_chrdev_region(rpmsg_major, RPMSG_RAW_MAX);

	return ret;
}
postcore_initcall(rpmsg_raw_init);

static void rpmsg_raw_exit(void)
{
	unregister_rpmsg_driver(&rpmsg_raw_driver);
	class_destroy(rpmsg_class);
	unregister_chrdev_region(rpmsg_major, RPMSG_RAW_MAX);
}
module_exit(rpmsg_raw_exit);

MODULE_ALIAS("rpmsg:rpmsg_raw");
