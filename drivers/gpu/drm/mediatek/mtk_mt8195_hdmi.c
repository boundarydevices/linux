...
void mtk_hdmi_clk_enable(struct mtk_hdmi *hdmi)
{
	...
	DRM_DEV_DEBUG_DRIVER(hdmi->dev, "[%s][%d],enable clock new\n", __func__, __LINE__);

	clk_set_parent(hdmi->clk[MTK_MT8195_HDMI_HDCP_SEL],
		       hdmi->clk[MTK_MT8195_HDMI_HDCP_PARENT]);

	for (i = 0; i < ARRAY_SIZE(mtk_hdmi_clk_names_mt8195); i++) {
		if (i == MTK_MT8195_HDMI_HDCP_PARENT)
			continue;
		else
...
void set_abist_pattern(struct device *dev, enum mtk_abist_pattern mode_num)
{
	...
	if (hdmi->enabled == true)
		drm_atomic_bridge_chain_disable(&hdmi->bridge, NULL);
	if (hdmi->powered == true)
		drm_atomic_bridge_chain_post_disable(&hdmi->bridge, NULL);

	msleep(50);

	drm_bridge_chain_mode_set(&hdmi->bridge, NULL, mode);
	drm_atomic_bridge_chain_pre_enable(&hdmi->bridge, NULL);
	drm_atomic_bridge_chain_enable(&hdmi->bridge, NULL);
