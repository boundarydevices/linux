...
static inline void dpu_sig_write(struct dpu_signature *sig, u32 value,
				 unsigned int offset)
{
...
void signature_shden(struct dpu_signature *sig, bool enable)
{
	...
	if (enable)
		...
		val &= ~SHDEN;
	dpu_sig_write(sig, val, STATICCONTROL);
...
void signature_shdldsel_global(struct dpu_signature *sig)
{
	...
	mutex_lock(&sig->mutex);
	val = dpu_sig_read(sig, STATICCONTROL);
	dpu_sig_write(sig, val | GLOBAL, STATICCONTROL);
	mutex_unlock(&sig->mutex);
}
...
signature_global_panic(struct dpu_signature *sig, unsigned int win, bool enable)
{
	...
	if (enable)
		...
		val &= ~ENGLOBALPANIC;
	dpu_sig_write(sig, val, EVALCONTRL(win));
	mutex_unlock(&sig->mutex);
}
...
{
	mutex_lock(&sig->mutex);
	dpu_sig_write(sig, XEVAL(xul)   | YEVAL(yul), EVALUPPERLEFT(win));
	dpu_sig_write(sig, XEVAL(--xlr) | YEVAL(--ylr), EVALLOWERRIGHT(win));
	mutex_unlock(&sig->mutex);
}
...
{
	mutex_lock(&sig->mutex);
	dpu_sig_write(sig, win_mask, SHADOWLOAD);
	mutex_unlock(&sig->mutex);
}
...
void signature_continuous_mode(struct dpu_signature *sig, bool enable)
{
	...
	if (enable)
		...
		val &= ~ENCONT;
	dpu_sig_write(sig, val, CONTINUOUSMODE);
	mutex_unlock(&sig->mutex);
}
...
{
	mutex_lock(&sig->mutex);
	dpu_sig_write(sig, KICK, SOFTWAREKICK);
	mutex_unlock(&sig->mutex);
}
