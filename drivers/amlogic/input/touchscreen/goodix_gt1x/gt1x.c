...
static int gt1x_parse_dt(struct device *dev)
{
    ...
    np = dev->of_node;
	gt1x_int_gpio = of_get_named_gpio(np, "irq-gpio", 0);
	gt1x_rst_gpio = of_get_named_gpio(np, "reset-gpio", 0);
...
ERR_GET_VCC:
    ...
    vdd_ana = NULL;
#endif // end of CONFIG_GT1X_POWER_CTRL
    return ret;
}
#endif
