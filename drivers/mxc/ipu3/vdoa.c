...
void vdoa_get_handle(vdoa_handle_t *handle)
{
	...
	*handle = (vdoa_handle_t *)NULL;
	...
	clk_prepare_enable(vdoa->ocram_clk);
	vdoa->state = VDOA_GET;
...
void vdoa_put_handle(vdoa_handle_t *handle)
{
	...
	clk_disable_unprepare(vdoa->ocram_clk);
	vdoa->state = VDOA_PUT;
	*handle = (vdoa_handle_t *)NULL;
...
static int vdoa_probe(struct platform_device *pdev)
{
	...
	vdoa->vdoa_clk = devm_clk_get(dev, clk_vdoa);
	if (IS_ERR(vdoa->vdoa_clk)) {
		dev_err(dev, "failed to get vdoa_clk\n");
		return PTR_ERR(vdoa->vdoa_clk);
	}
	vdoa->ocram_clk = devm_clk_get(dev, clk_ocram);
	if (IS_ERR(vdoa->ocram_clk)) {
		dev_err(dev, "failed to get ocram_clk\n");
		return PTR_ERR(vdoa->ocram_clk);
	}
