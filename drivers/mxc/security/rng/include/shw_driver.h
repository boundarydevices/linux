...
typedef enum fsl_shw_auth_ctx_flags_t {
	...
	FSL_ACCO_NIST_CCM = 0x10,
				/**< Formatting of CCM input data is
...
typedef struct shw_queue_entry_t {
	...
	void *user_mode_req;	    /**<  Pointer into user space.  */
	 uint32_t(*postprocess) (struct shw_queue_entry_t *q);
							  /**< (internal)
                                                           function to call
                                                           when this operation
                                                           completes.
...
typedef struct shw_queue_t {
	struct shw_queue_entry_t *head;
				    /**< First entry in queue. */
	struct shw_queue_entry_t *tail;
				    /**< Last entry. */
} shw_queue_t;
...
typedef struct fsl_shw_uco_t {
	...
	os_process_handle_t process;
				  /**< remember for signalling User mode */
	fsl_shw_spo_t *partition;/**< chain of secure partitions owned by
                                          the user */
#endif
	struct fsl_shw_uco_t *next;
				/**< To allow user-mode chaining of contexts,
...
typedef struct fsl_shw_pco_t {
	...
	fsl_shw_key_alg_t *sym_algorithms;
					/**< Pointer to array. */
	unsigned sym_mode_count;/**< Number of sym_modes. */
	fsl_shw_sym_mode_t *sym_modes;
				   /**< Pointer to array. */
	unsigned hash_algorithm_count;
				   /**< Number of hash_algorithms. */
	fsl_shw_hash_alg_t *hash_algorithms;
					 /**< Pointer to array */
	uint8_t sym_support[5][4];
				/**< indexed by key alg then mode */
