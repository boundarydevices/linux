...
static int init_display(struct fbtft_par *par)
{
	...
	if (par->irq) {
		write_reg(par, MIPI_DCS_SET_TEAR_ON, 0x00);	/* Turn on TE output */
		write_reg(par, MIPI_DCS_SET_TEAR_SCANLINE, (te_line >> 8), (te_line & 0xff));	/* Trial & error to get best starting  */
	}
