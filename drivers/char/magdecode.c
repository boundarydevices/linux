...
static void data_timer(struct timer_list *t)
{
	unsigned long flags;
	struct mag_dev *dev = from_timer(dev, t, data_timer);
...
{
	unsigned long flags;
	struct mag_dev *dev = from_timer(dev, t, front_timer);
...
static void rear_timer(struct timer_list *t)
{
	unsigned long flags;
	struct mag_dev *dev = from_timer(dev, t, rear_timer);
...
static int mag_open(struct inode *inode, struct file *file)
{
	...
	if (!dev->open_count) {
		...
		timer_setup(&dev->data_timer, data_timer, 0);
