...
int proc_get_dummy(char *page, char **start, off_t offset, int count, int *eof, void *data)
{	
	*eof = 1;
	return 0;
}
			  ...
			  int *eof, void *data)
{
	...
	len += snprintf(page + len, count - len, "name=%s, iface_type=%d, bSurpriseRemoved=%d, bDriverStopped=%d\n",
						dev->name, padapter->iface_type,
						padapter->bSurpriseRemoved, padapter->bDriverStopped);
#else
	len += snprintf(page + len, count - len, "name=%s, bSurpriseRemoved=%d, bDriverStopped=%d\n",
...
int proc_get_rx_logs(char *page, char **start,
			  off_t offset, int count,
			  int *eof, void *data)
{
	int i;
...
int proc_get_tx_logs(char *page, char **start,
			  off_t offset, int count,
			  int *eof, void *data)
{
	int i;
...
int proc_get_int_logs(char *page, char **start,
			  off_t offset, int count,
			  int *eof, void *data)
{
	int i;
...
int proc_set_fwdl_test_case(struct file *file, const char *buffer, unsigned long count, void *data)
{
	struct net_device *dev = (struct net_device *)data;
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	char tmp[32];
