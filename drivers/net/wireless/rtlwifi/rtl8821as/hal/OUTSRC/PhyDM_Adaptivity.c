...
)
{
	...
	while (bAdjust2) {
		Phydm_SetEDCCAThreshold(pDM_Odm, pDM_Odm->H2L_lb, pDM_Odm->L2H_lb);
		...
		if (txEdcca1 > 1) {
			pDM_Odm->Adaptivity_IGI_upper = pDM_Odm->Adaptivity_IGI_upper - 1;
			pDM_Odm->H2L_lb = pDM_Odm->H2L_lb + 1;
			pDM_Odm->L2H_lb = pDM_Odm->L2H_lb + 1;
		} else
			bAdjust2 = FALSE;
	}
