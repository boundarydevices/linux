...
_func_enter_;
	...
	_memset((void*)pxmitpriv, 0, sizeof(struct xmit_priv));
	...
	*/
	size = NR_XMITFRAME * sizeof(struct xmit_frame) + 4;
	pxmitpriv->pallocated_frame_buf = _malloc(size);
	if (pxmitpriv->pallocated_frame_buf  == NULL){
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("alloc xmit_frame fail!\n"));
		pxmitpriv->pxmit_frame_buf = NULL;
		goto fail_init;
	}
	_memset(pxmitpriv->pallocated_frame_buf, 0, size);
	pxmitpriv->pxmit_frame_buf = ALIGN_ADDR(pxmitpriv->pallocated_frame_buf, 4);
...
//		pxframe->pxmitbuf = NULL;
	...
	*/
	...
	size = NR_XMITBUFF * sizeof(struct xmit_buf) + 4;
	pxmitpriv->pallocated_xmitbuf = _malloc(size);
	if (pxmitpriv->pallocated_xmitbuf  == NULL){
		RT_TRACE(_module_rtl871x_xmit_c_,_drv_err_,("alloc xmit_buf fail!\n"));
		goto fail_init;
	}
	_memset(pxmitpriv->pallocated_xmitbuf, 0, size);
	pxmitpriv->pxmitbuf = ALIGN_ADDR(pxmitpriv->pallocated_xmitbuf, 4);

	pxmitbuf = (struct xmit_buf*)pxmitpriv->pxmitbuf;
	size = MAX_XMITBUF_SZ + XMITBUF_ALIGN_SZ;
	for (i = 0; i < NR_XMITBUFF; i++)
	{
		pxmitbuf->pallocated_buf = _malloc(size);
		if (pxmitbuf->pallocated_buf == NULL) {
			res = _FAIL;
			goto fail_init;
		}
		_memset(pxmitbuf->pallocated_buf, 0, size);
		pxmitbuf->pbuf = ALIGN_ADDR(pxmitbuf->pallocated_buf, XMITBUF_ALIGN_SZ);
	...
	// SDIO allocate hwxmits in xmit_thread()
	if (alloc_hwxmits(padapter) == _FAIL) goto fail_init;
...
{
	u8 bool;
	// If driver xmit ARP packet, driver can set ps mode to initial setting.
	// It stands for getting DHCP or fix IP.
	if (pattrib->ether_type == 0x0806)
	{
//		printk(KERN_DEBUG "======================update_attrib: sending ARP Packet\n");
	...
	else if (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE)
	{
		...
		if (pattrib->ether_type != 0x8712) {
			...
			RT_TRACE(_module_rtl871x_xmit_c_, _drv_debug_,
				 ("update_attrib: [MP] ether_type(0x%04x) != 0x8712!\n",
				 pattrib->ether_type));
			goto exit;
		}
		...
		if (pktfile.pkt_len > 282) {//MINIMUM_DHCP_PACKET_SIZE) {
			if (ETH_P_IP == pattrib->ether_type) {// IP header
				if (((tmp[21] == 68) && (tmp[23] == 67)) ||
					...
					printk(KERN_DEBUG "======================update_attrib:sending DHCP Packet\n");
...
#endif
	if(pattrib->ether_type==0x86dd){
	//	printk("===========[drop IPV6 packet]===========packet =%x\n",pattrib->ether_type);
		res =_FAIL;
		goto exit;
	}
...
#ifdef CONFIG_RTL8712_TCP_CSUM_OFFLOAD_TX
{
	...
	if (skb->ip_summed == CHECKSUM_PARTIAL) {
		if (skb_shinfo(skb)->nr_frags == 0) {
				...
				printk(KERN_ERR "%s-%d TCP CSUM offload Error!!\n", __FUNCTION__, __LINE__);
	    ...
	    (check_fwstate(pmlmepriv, WIFI_MP_LPBK_STATE) == _TRUE)*/)
	{
		...
		if (pattrib->ether_type == 0x8712) {
			...
			_pktfile_read(&pktfile, NULL, TXDESC_SIZE); // take care update_txdesc overwrite this
			_memcpy(ptxdesc, pxmitframe->mp_buf + pattrib->pkt_hdrlen, TXDESC_SIZE);
#else
	...
	while (1)
	{
...
#ifdef MP_XMIT_COPY_PKT
		if (check_fwstate(pmlmepriv, WIFI_MP_STATE) == _TRUE) {
			u8* mp_buf = pxmitframe->mp_buf + (pktfile.cur_addr - pktfile.buf_start);
			if (bmcst) {
				mem_sz = _pktfile_read(&pktfile, NULL, pattrib->pktlen);
			} else {
				mem_sz = _pktfile_read(&pktfile, NULL, mpdu_len);
			}
			_memcpy(pframe, mp_buf, mem_sz);
			_mfree(pxmitframe->mp_buf, pktfile.buf_len);
			pxmitframe->mp_buf = NULL;
		} else
#endif
		if (bmcst) {
		...
		if (bmcst || (endofpktfile(&pktfile) == _TRUE))
		{
			...
			if (pattrib->encrypt==_TKIP_)
				pframe+=8;  //reserve  the mic code size.
...
void free_hwxmits(_adapter *padapter)
{
	...
	if (phwxmit == NULL) return;

	entry = pxmitpriv->hwxmit_entry;
	for (i = 0; i < entry; i++, phwxmit++)
		_spinlock_free(&phwxmit->xmit_lock);
