...
static u8 efuse_one_byte_read(_adapter *padapter, u16 addr, u8 *data)
{
	...
	cmd = read32(padapter, EFUSE_CTRL);
	cmd &= 0x00FC0000; // Keep some original setting, erase others
	cmd |= ((addr & 0x3FF) << 8); // address
	cmd |= 0x72000000;
	write32(padapter, EFUSE_CTRL, cmd);
#else
