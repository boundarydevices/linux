...
s32 init_mp_priv (struct mp_priv *pmp_priv)
{
...
s32 init_mp_priv(struct mp_priv *pmp_priv)
{
	...
	i = NR_MP_XMITFRAME * sizeof(struct mp_xmit_frame) + 4;
	pmp_priv->pallocated_mp_xmitframe_buf = _malloc(i);
	if (pmp_priv->pallocated_mp_xmitframe_buf == NULL) return _FAIL;
	_memset(pmp_priv->pallocated_mp_xmitframe_buf, 0, i);
	pmp_priv->pmp_xmtframe_buf = ALIGN_ADDR(pmp_priv->pallocated_mp_xmitframe_buf, 4);
...
s32 mp871xinit(_adapter *padapter)
{
	struct mp_priv *pmppriv = &padapter->mppriv;

	pmppriv->papdater = padapter;
	return init_mp_priv(pmppriv);
}
...
#define IOCMD2NO(iocmd) ((iocmd.cmdclass << 24) | (iocmd.value << 8) | iocmd.index)
...
u32 bb_reg_read(PADAPTER pAdapter, u16 offset)// offset : 0X800~0XFFF
{
	...
	cmdno = IOCMD2NO(iocmd);
	fw_iocmd_read(pAdapter, cmdno, &bb_val);
	...
	if (shift != 0) {
		...
		iocmd.value += 4;
		cmdno = IOCMD2NO(iocmd);
		fw_iocmd_read(pAdapter, cmdno, &bb_val2);
		bb_val2 <<= ((4 - shift) * 8);
		bb_val |= bb_val2;
	}
...
u8 bb_reg_write(PADAPTER pAdapter, u16 offset, u32 value)// offset : 0X800~0XFFF
{
	...
	if (shift != 0) {
		...
		if (fw_iocmd_write(pAdapter, cmdno, value) == _FALSE)
			return _FALSE;
	...
	return fw_iocmd_write(pAdapter, cmdno, value);
}
...
u32 rf_reg_read(PADAPTER pAdapter, u8 path, u8 offset) // offset : 0x00 ~ 0xFF
{
	...
	fw_iocmd_read(pAdapter, cmdno, &rf_data);
...
u32 get_bb_reg(PADAPTER pAdapter, u16 offset, u32 bitmask)
{
	...
	RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
		 ("get_bb_reg: offset=0x%03X mask=0x%08X val=0x%08X\n",
		  offset, bitmask, org_value));
...
u8 set_bb_reg(PADAPTER pAdapter, u16 offset, u32 bitmask, u32 value)
{
	...
	if (bitmask != bMaskDWord) {
		...
		RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
			 ("set_bb_reg: offset=0x%03X org=0x%08X new=0x%08X\n",
			  offset, org_value, new_value));
	} else {
		new_value = value;
		RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
			 ("set_bb_reg: offset=0x%03X value=0x%08X\n",
			  offset, new_value));
	}
...
u32 get_rf_reg(PADAPTER pAdapter, u8 path, u8 offset, u32 bitmask)
{
	...
	RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
		 ("get_rf_reg: path=%d offset=0x%02X mask=0x%05X val=0x%05X\n",
		  path, offset, bitmask, org_value));
	bit_shift = bitshift(bitmask);
	new_value = (org_value & bitmask) >> bit_shift;
...
u8 set_rf_reg(PADAPTER pAdapter, u8 path, u8 offset, u32 bitmask, u32 value)
{
	...
	if (bitmask != bRFRegOffsetMask) {
		...
		new_value = ((org_value & (~bitmask)) | (value << bit_shift));
		RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
			 ("set_rf_reg: path=%d offset=0x%02X v=0x%05X org=0x%05X new=0x%05X\n",
			  path, offset, value, org_value, new_value));
	} else {
		new_value = value;
		RT_TRACE(_module_rtl871x_mp_c_, _drv_notice_,
			 ("set_rf_reg: path=%d offset=0x%02X v=0x%05X\n",
			  path, offset, new_value));
	}
...
{
#if 0
	fw_iocmd_read(pAdapter, IOCMD_GET_THERMAL_METER, value);
	*value &= 0xFF;
#else
...
void SetSingleToneTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart)// Start Single Tone.
	{
		...
		set_rf_reg(pAdapter, rfPath, RF_TX_G2, bRFRegOffsetMask, 0xd4000);
		usleep_os(100);
		set_rf_reg(pAdapter, rfPath, RF_AC, bRFRegOffsetMask, 0x20010);
		usleep_os(100);
	}
	else// Stop Single Tone.
	{
		...
		//set_bb_reg(pAdapter, rFPGA0_RFMOD, bOFDMTxDACPhase, 0x1);
		set_rf_reg(pAdapter, rfPath, RF_TX_G2, bRFRegOffsetMask, 0x54000);
		usleep_os(100);
		set_rf_reg(pAdapter, rfPath, RF_AC, bRFRegOffsetMask, 0x30000); // PAD all on.
		usleep_os(100);
	}
#endif
}
...
void SetCarrierSuppressionTx(PADAPTER pAdapter, u8 bStart)
{
	...
	if (bStart) // Start Carrier Suppression.
	{
		...
		if (pAdapter->mppriv.curr_rateidx <= MPT_RATE_11M) {
			...
			set_bb_reg(pAdapter, rCCK0_System, bCCKScramble, bDisable);  //turn off scramble setting
	...
	else// Stop Carrier Suppression.
	{
		...
		if (pAdapter->mppriv.curr_rateidx <= MPT_RATE_11M ) {
			...
			set_bb_reg(pAdapter, rCCK0_System, bCCKScramble, bEnable);  //turn on scramble setting
