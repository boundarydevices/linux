...
static s32 mp_start_test(_adapter *padapter)
{
	...
	// reference from surveydone_event_callback() & update_registrypriv_dev_network()
	...
	bssid.InfrastructureMode = Ndis802_11IBSS; // WIRELESS_11B
	bssid.Configuration.DSConfig = 1; // channel
	...
	//3 2. join psudo AdHoc
	// reference from joinbss_event_callback()
	// clear psta in the cur_network, if any
	psta = get_stainfo(&padapter->stapriv, cur_network->network.MacAddress);
	if (psta) free_stainfo(padapter, psta);

	cur_network->join_res = 1;
	cur_network->aid = 1;
	_memcpy(&cur_network->network, &bssid, length);

	//3 3. create a new psta
	// reference from stassoc_event_callback()
...
_func_enter_;
	...
	RT_TRACE(_module_rtl871x_mp_ioctl_c_, _drv_notice_, ("+oid_rt_pro_set_single_tone_tx_hdl\n"));
...
#ifdef POWER_DOWN_SUPPORT
	if (_r8712_power_off(Adapter, !bpwrup) == 0)
