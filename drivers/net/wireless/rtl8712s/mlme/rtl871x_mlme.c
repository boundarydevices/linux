...
_func_enter_;
	...
	delta_time = (curr_time - (u32)pnetwork->last_scanned) / 10;
...
_func_enter_;
	...
	while(1)
	{
		...
		{
			if (oldest == NULL ||time_after((unsigned long)oldest->last_scanned, (unsigned long)pwlan->last_scanned))
				oldest = pwlan;
		}
...
_func_enter_;
	...
	while(1)
	{
		...
		if ((oldest == ((struct wlan_network *)0)) ||
		    time_after((unsigned long)oldest->last_scanned, (unsigned long)pnetwork->last_scanned))
			oldest = pnetwork;
...
/*
 * indicate_disconnect: the caller has to lock pmlmepriv->lock
 */
...
_func_enter_;
	if(sizeof(_list) == 4*sizeof(u32))
	{
		pnetwork = (struct wlan_network *)_malloc(sizeof(struct wlan_network));
		_memcpy((u8*)pnetwork+16,(u8*)pbuf+8,sizeof(struct wlan_network)-16);
	}
	else
		pnetwork = (struct wlan_network *)pbuf;
...
//			if (ptarget_wlan)	// check above
			{
				...
				switch (pnetwork->network.InfrastructureMode)
				{
					...
					case Ndis802_11IBSS:
						...
						RT_TRACE(_module_rtl871x_mlme_c_,_drv_err_,("IBBBBBSSS mode ~~\n"));
						pmlmepriv->fw_state = WIFI_ADHOC_STATE;
						break;
					default:
...
ignore_joinbss_callback:
	...
	if(sizeof(_list) == 4*sizeof(u32))
	{
		_mfree((u8*)pnetwork,sizeof(struct wlan_network));
	}
...
_func_enter_;	
	...
	printk(KERN_INFO "[%s] mac = %X %X %X %X %X %X, seq = %d, tid = %d\n", __FUNCTION__,
...
_func_enter_;
	...
	sz = generate_ie(pregistrypriv,adapter);
...
//the fucntion is >= passive_level
unsigned int add_ht_addt_info(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len)
{
	u32 ielen, out_len =  0;
	unsigned char *p, *pframe;
...
//	phtpriv->ht_option = 0;
	...
	if (p && (ielen>0))
	{
		...
		RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("add_ht_addt_info: p=0x%p ielen=%d\n", p,ielen));
	
	}
	else{
		if(p==NULL){
			_memset(&ht_addt_info, 0, sizeof(struct ieee80211_ht_addt_info));
			//need to add the HT additional IEs
			ht_addt_info.control_chan=pregistrypriv->channel;
			pframe = set_ie(out_ie+out_len, _HT_ADD_INFO_IE_,
							sizeof(struct ieee80211_ht_addt_info), (unsigned char*)&ht_addt_info, pout_len);	
			RT_TRACE(_module_rtl871x_mlme_c_,_drv_info_,("add_ht_addt_info:pregistrypriv->channel=%d\n", pregistrypriv->channel));
