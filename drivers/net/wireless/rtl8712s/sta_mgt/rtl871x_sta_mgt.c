...
static void _init_sta_xmit_priv(struct sta_xmit_priv *psta_xmitpriv)
{
_func_enter_;

	_memset((u8*)psta_xmitpriv, 0, sizeof(struct sta_xmit_priv));
...
static void _init_sta_recv_priv(struct sta_recv_priv *psta_recvpriv)
	{
_func_enter_;
...
s32 _init_sta_priv(struct sta_priv *pstapriv)
{
...
_func_enter_;
	...
	i = sizeof(struct sta_info) * NUM_STA + 4;
	pstapriv->pallocated_stainfo_buf = _malloc(i);
	if (pstapriv->pallocated_stainfo_buf == NULL) {
		RT_TRACE(_module_rtl871x_sta_mgt_c_,_drv_err_,("_init_sta_priv: alloc stainfo_buf FAIL!\n"));
		return _FAIL;
	}
	_memset(pstapriv->pallocated_stainfo_buf, 0, i);
	pstapriv->pstainfo_buf = ALIGN_ADDR(pstapriv->pallocated_stainfo_buf, 4);
...
void _free_sta_priv(struct sta_priv *pstapriv)
{
_func_enter_;

	if (pstapriv == NULL) return;
