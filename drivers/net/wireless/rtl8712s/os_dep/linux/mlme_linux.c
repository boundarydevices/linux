...
_func_enter_;	
	...
	*/
	...
	if(adapter->bclosed==_FALSE)
		indicate_wx_assoc_event(adapter);

	netif_carrier_on(adapter->pnetdev);

	if(!netif_queue_stopped(adapter->pnetdev))
                netif_start_queue(adapter->pnetdev);
        else
...
void clear_security_priv( _adapter *adapter){
...
_func_enter_;
   ...
   netif_carrier_off(adapter->pnetdev);
	if (!netif_queue_stopped(adapter->pnetdev))
        	  netif_stop_queue(adapter->pnetdev);
