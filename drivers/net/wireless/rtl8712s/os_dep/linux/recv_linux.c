...
_func_enter_;
...
#ifdef NET_SKBUFF_DATA_USES_OFFSET
	RT_TRACE(_module_recv_osdep_c_,_drv_err_,("skb->head=%p skb->data=%p skb->tail=%u skb->end=%u skb->len=%d\n", skb->head, skb->data, skb->tail, skb->end, skb->len));
#else
	RT_TRACE(_module_recv_osdep_c_,_drv_info_,("skb->head=%p skb->data=%p skb->tail=%p skb->end=%p skb->len=%d\n", skb->head, skb->data, skb->tail, skb->end, skb->len));
#endif
	...
	if(padapter->bSurpriseRemoved==_TRUE){
		printk("\n recv_indicatepkt  padapter->bSurpriseRemoved==_TRUE ,  drop packet~~\n");
		goto _recv_indicatepkt_drop;
	}
	...
	RT_TRACE(_module_recv_osdep_c_,_drv_info_,("recv_indicatepkt :after netif_rx!!!!\n"));
