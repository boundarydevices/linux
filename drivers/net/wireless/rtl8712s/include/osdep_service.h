...
#define _FALSE		0
	...
	#define   _usb_alloc_urb(x,y)       usb_alloc_urb(x)
#else
	#define   _usb_alloc_urb(x,y)       usb_alloc_urb(x,y)
#endif

#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,5,4))
	#define   _usb_submit_urb(x,y)     usb_submit_urb(x)
#else
...
static __inline void*  _netdev_priv( struct net_device*  dev )
{
...
static __inline void os_free_netdev(struct net_device*  dev)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2,5,0))
		free_netdev(dev);
#else
...
{
	if (ptimer) del_timer_sync(ptimer);
...
static __inline void _cancel_timer(_timer *ptimer,u8 *bcancelled)
{
	if (ptimer) NdisMCancelTimer(ptimer, bcancelled);
}
