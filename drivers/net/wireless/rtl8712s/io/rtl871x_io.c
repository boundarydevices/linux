...
_func_enter_;
	...
	if (pio_queue == NULL) {
		RT_TRACE(_module_rtl871x_ioctl_c_,_drv_err_,("alloc_io_queue: pio_queue == NULL !!!!\n"));
	...
	if (pio_queue->pallocated_free_ioreqs_buf == NULL) {
		RT_TRACE(_module_rtl871x_io_c_,_drv_err_,("alloc_io_queue: pio_queue->pallocated_free_ioreqs_buf == NULL!!!!\n"));
		goto alloc_io_queue_fail;
	}
	_memset(pio_queue->pallocated_free_ioreqs_buf, 0, NUM_IOREQ * sizeof(struct io_req) + 4);

	pio_queue->free_ioreqs_buf = ALIGN_ADDR(pio_queue->pallocated_free_ioreqs_buf, 4);
	...
	if (register_intf_hdl((u8*)adapter, &pio_queue->intf) == _FAIL) {
		RT_TRACE(_module_rtl871x_ioctl_c_,_drv_err_,("alloc_io_queue: register_intf_hdl == _FAIL !!!!\n"));
		goto alloc_io_queue_fail;
	}
...
static void mfree_io_queue_lock(struct io_queue *pio_queue)
{
	u32 i;
	struct io_req *pio_req;


	if (pio_queue == NULL) return;
...
#if defined(PLATFORM_OS_XP) && defined(CONFIG_SDIO_HCI)
		if (pio_req->pirp) {
			IoFreeIrp(pio_req->pirp);
			pio_req->pirp = NULL;
		}
		if (pio_req->pmdl) {
			IoFreeMdl(pio_req->pmdl);
			pio_req->pmdl = NULL;
		}
		if (pio_req->sdrp) {
			ExFreePool(pio_req->sdrp);
			pio_req->sdrp = NULL;
		}
#endif
...
_func_enter_;
	...
	pio_queue = (struct io_queue*)adapter->pio_queue;
	if (pio_queue == NULL) return;
	...
	if (pio_queue->pallocated_free_ioreqs_buf) {
		...
		pio_queue->pallocated_free_ioreqs_buf = NULL;
	}
