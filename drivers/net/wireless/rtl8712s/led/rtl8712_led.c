			...
			if(bStopBlinking)
			{
				...
				else if(check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE)
				{
					...
					if( pLed->bLedOn )
						...
						pLed->BlinkingLedState = LED_ON;
					_set_timer(&(pLed->BlinkTimer), LED_BLINK_LINK_INTERVAL_ALPHA);
					RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("CurrLedState %lu\n", pLed->CurrLedState));
	...
	)
{
	...
	{
		SwLedOn(padapter, pLed);
		RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("Blinktimes (%lu): turn on\n", pLed->BlinkTimes));
	}
	else
	{
		SwLedOff(padapter, pLed);
		RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("Blinktimes (%lu): turn off\n", pLed->BlinkTimes));
	}
		...
		case LED_SCAN_BLINK:
			...
			if(bStopBlinking)
			{
				...
				else if(check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE)
				{
					...
					pLed->BlinkingLedState = LED_ON;
					SwLedOn(padapter, pLed);
					RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("stop scan blink CurrLedState %lu\n", pLed->CurrLedState));
		...
		case LED_TXRX_BLINK:
			...
			if(bStopBlinking)
			{
				...
				else if(check_fwstate(pmlmepriv, _FW_LINKED)== _TRUE)
				{
					...
					pLed->BlinkingLedState = LED_ON;
					SwLedOn(padapter, pLed);
					RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("stop CurrLedState %lu\n", pLed->CurrLedState));
	...
	)
{
	...
	RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("SwLedBlink4 CurrLedState %lu\n", pLed->CurrLedState));
	...
	)
{
	...
	RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("SwLedBlink5 CurrLedState %lu\n", pLed->CurrLedState));
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("Led %lu\n", pLed->CurrLedState));
...
)
{
	...
	RT_TRACE(_module_rtl8712_led_c_,_drv_info_,("ledcontrol 6 Led %lu\n", pLed->CurrLedState));
}
	...
	)
{
	...
	if( ledpriv == NULL || ledpriv->bRegUseLed == _FALSE)
		return;
