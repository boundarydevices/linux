...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("RTL8712 Wireless Lan Driver");
...
struct net_device *init_netdev(void)
{
	...
	padapter = (_adapter *) _netdev_priv(pnetdev);
	padapter->pnetdev = pnetdev;

	//pnetdev->init = NULL;
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,29))
	printk(KERN_INFO "%s: register rtl8712_netdev_ops to netdev_ops\n", __func__);
...
void enable_video_mode( _adapter* padapter, int cbw40_value)
{
        ...
        fw_iocmd(padapter, intcmd);
}

static int netdev_open(struct net_device *pnetdev)
{
	uint status;
	PADAPTER padapter = (PADAPTER)_netdev_priv(pnetdev);


//	printk("+%s: interface UP. bup=%d\n", __func__, padapter->bup);
	...
	padapter->bclosed=_FALSE;

//	printk("-%s: UP OK. bup=%d\n", __func__, padapter->bup);
	RT_TRACE(_module_os_intfs_c_,_drv_info_,("-871x_drv - dev_open\n"));
...
//	printk(KERN_INFO "+%s: interface DOWN! bup=%d hw_init_completed=%d\n",
//		__func__, padapter->bup, padapter->hw_init_completed);
