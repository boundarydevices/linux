...
s32 _init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{	
	s32 i;
...
_func_enter_;		
	...
	_memset((u8*)precvpriv, 0, sizeof(struct recv_priv));
	...
	res = os_recv_resource_init(precvpriv, padapter);
	if (res == _FAIL) goto fail_init;

	precvpriv->pallocated_frame_buf = _malloc(NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ);
	if (precvpriv->pallocated_frame_buf == NULL) goto fail_init;
	_memset(precvpriv->pallocated_frame_buf, 0, NR_RECVFRAME * sizeof(union recv_frame) + RXFRAME_ALIGN_SZ);
	precvpriv->precv_frame_buf = ALIGN_ADDR(precvpriv->pallocated_frame_buf, RXFRAME_ALIGN_SZ);
...
_func_enter_;	
	...
	if(prxattrib->encrypt ==_TKIP_)
        {
		RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("recvframe_chkmic:prxattrib->encrypt ==_TKIP_\n"));
		RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("recvframe_chkmic:da=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
		...
		if(stainfo!= NULL)
			...
			if(IS_MCAST(prxattrib->ra))
				...
				RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("recvframe_chkmic: bcmc key\n"));
				if(psecuritypriv->binstallGrpkey==_FALSE)
                                {
					res=_FAIL;
					RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("recvframe_chkmic:didn't install group key!\n"));
					goto exit;
				}
			}
			else{
				mickey=&stainfo->dot11tkiprxmickey.skey[0];
				RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("recvframe_chkmic: unicast key\n"));
			}
			datalen=precvframe->u.hdr.len-prxattrib->hdrlen-prxattrib->iv_len-prxattrib->icv_len-8;//icv_len included the mic code
			pframe=precvframe->u.hdr.rx_data;
			payload=pframe+prxattrib->hdrlen+prxattrib->iv_len;
			RT_TRACE(_module_rtl871x_recv_c_,_drv_info_,("prxattrib->iv_len=%d prxattrib->icv_len=%d\n",prxattrib->iv_len,prxattrib->icv_len));
			...
			for(i=0;i<8;i++){
				if(miccode[i] != *(pframemic+i)){
					RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("recvframe_chkmic:miccode[%d](%02x) != *(pframemic+%d)(%02x)\n",i,miccode[i],i,*(pframemic+i)));
					bmic_err=_TRUE;
				}
			}
			if(bmic_err==_TRUE){
				RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("*(pframemic-8)-*(pframemic-1)=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
					*(pframemic-8),*(pframemic-7),*(pframemic-6),*(pframemic-5),*(pframemic-4),*(pframemic-3),*(pframemic-2),*(pframemic-1)));
				RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("*(pframemic-16)-*(pframemic-9)=0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x:0x%02x\n",
					*(pframemic-16),*(pframemic-15),*(pframemic-14),*(pframemic-13),*(pframemic-12),*(pframemic-11),*(pframemic-10),*(pframemic-9)));

				{
					uint i;
					RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("======demp packet (len=%d)======\n",precvframe->u.hdr.len));
					...
					RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("======demp packet end [len=%d]======\n",precvframe->u.hdr.len));
					RT_TRACE(_module_rtl871x_recv_c_,_drv_err_,("hrdlen=%d\n",prxattrib->hdrlen));
				}
...
_func_enter_;		
	...
	pkt_size = precvframe->u.hdr.len;

	padapter->ledpriv.LedControlHandler(padapter, LED_CTL_RX);

#if defined(CONFIG_SDIO_HCI) && defined(PLATFORM_WINDOWS)
