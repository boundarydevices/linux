...
#ifdef CONFIG_PWRCTRL
	set_ps_mode(padapter,PS_MODE_MIN, padapter->registrypriv.smart_ps);
...
_func_enter_;
	...
	if (check_fwstate(pmlmepriv, _FW_LINKED) == _TRUE)
	{
		...
		printk("\n set_bssid: bssid is the same   ==>skip~~~fw_state=0x%08x   try again\n", get_fwstate(pmlmepriv));
...
//		indicate_disconnect(padapter);
	...
	printk(KERN_DEBUG "do_join in %s\n",__FUNCTION__); //Jeff
...
#ifndef PLATFORM_ANDROID // why use this?? (Jeff?)
			disassoc_cmd(padapter);
...
//#ifdef  PLATFORM_ANDROID // why use this?? (Jeff?)
//
//	printk("=====set_802_11_ssid: ssid=[%s] fw_state=0x%08x skip\n",
//		  ssid->Ssid, get_fwstate(pmlmepriv));
//	goto _Abort_Set_SSID;
//#endif
	status = do_join(padapter);
	...
	if (*pold_state != networktype)
	{
		...
		*pold_state = networktype;
		...
		switch (networktype)
		{
			case Ndis802_11IBSS:
				set_fwstate(pmlmepriv, WIFI_ADHOC_STATE);
				padapter->pwrctrlpriv.force_active = _TRUE;
				break;

			case Ndis802_11Infrastructure:
				set_fwstate(pmlmepriv, WIFI_STATION_STATE);
				padapter->pwrctrlpriv.force_active = _FALSE;
				break;
...
#ifdef CONFIG_PWRCTRL
		if(padapter->pwrctrlpriv.force_active == _TRUE)
		{
