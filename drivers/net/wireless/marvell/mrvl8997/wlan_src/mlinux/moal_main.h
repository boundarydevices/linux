...
/** channel switch wait queue token */
    ...
    /** If link layer in host is enabled */
	int link_layer_en;
#if CFG80211_VERSION_CODE >= KERNEL_VERSION(3, 8, 0)
	/** Moal Private structure on which assoc rsp received**/
	moal_private *assoc_resp_priv;
...
mlan_status woal_get_power_info(moal_private *priv, t_u8 wait_option,
...
#define FW_DUMP_INFO_LEN                      0x200000
/** mem dump header */
typedef struct {
    /** seq number */
	t_u16 seq_num;
...
t_void woal_store_dpd_log(moal_handle *phandle, mlan_event *pmevent);
...
mlan_status woal_dpdlog_enble(moal_private *priv, t_u16 *enable,
			      t_u8 wait_option, t_u8 action);
mlan_status woal_get_dpd_info(moal_private *priv, t_u8 wait_option,
			      mlan_ds_get_dpdlog * dpdlog);
