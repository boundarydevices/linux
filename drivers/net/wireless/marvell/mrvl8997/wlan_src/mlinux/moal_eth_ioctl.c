 ...
 */
int
woal_priv_dpdlog_enable(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int ret = 0;
	...
	header_len = strlen(CMD_NXP) + strlen(PRIV_CMD_DPDLOG_ENABLE);
	if (strlen(respbuf) == header_len) {
		/* GET operation */
		user_data_len = 0;
	} else {
	...
	if (MLAN_STATUS_SUCCESS !=
	    woal_dpdlog_enble(priv, &enable, MOAL_IOCTL_WAIT, action)) {
		ret = -EFAULT;
		goto done;
	}
 ...
 */
int
woal_get_priv_get_dpdlog(moal_private *priv, t_u8 *respbuf, t_u32 respbuflen)
{
	int ret = 0;
	mlan_ds_get_dpdlog *dpd_info;
	ENTER();

	if (respbuflen < sizeof(mlan_ds_get_dpdlog)) {
		PRINTM(MERROR, "Get dpdlog: respbuflen (%d) too small!",
		       (int)respbuflen);
		ret = -EFAULT;
		goto done;
	}
	dpd_info = (mlan_ds_get_dpdlog *) respbuf;
	if (MLAN_STATUS_SUCCESS !=
	    woal_get_dpd_info(priv, MOAL_IOCTL_WAIT, dpd_info)) {
		PRINTM(MERROR, "Get DPD log: Failed to get dpd info!");
		ret = -EFAULT;
		goto done;
	}

	ret = sizeof(mlan_ds_get_dpdlog);
			      ...
			      t_u32 respbuflen)
{
	...
	if (tx_rx_info->enable & GET_TX_RX_HISTOGRAM) {
		...
		memcpy(pos, &misc->param.tx_rx_histogram.value,
			     ...
			     t_u32 respbuflen)
{
	...
	if (strlen(respbuf) == header_len) {
		...
		if (data[0] > TX_AMPDU_DISABLE_PROTECTION) {
	...
	if (strncmp(buf, CMD_NXP, strlen(CMD_NXP)) == 0) {
		...
		if (strnicmp
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_DPDLOG_ENABLE,
			    strlen(PRIV_CMD_DPDLOG_ENABLE)) == 0) {
			/* get/set dpdlog enable */
			len = woal_priv_dpdlog_enable(priv, buf,
						      priv_cmd.total_len);
			goto handled;
		} else if (strnicmp
			   (buf + strlen(CMD_NXP), PRIV_CMD_GET_DPDLOG,
			    strlen(PRIV_CMD_GET_DPDLOG)) == 0) {
			/* Get dpdlog */
			len = woal_get_priv_get_dpdlog(priv, buf,
						       priv_cmd.total_len);
			goto handled;
		} else if (strnicmp
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_DRCS_CFG,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_MULTI_CHAN_CFG,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_MULTI_CHAN_POLICY,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_NET_MON,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_FW_WAKEUP_METHOD,
			...
			goto handled;
			   ...
			   (buf + strlen(CMD_NXP), PRIV_CMD_GET_CORRELATED_TIME,
