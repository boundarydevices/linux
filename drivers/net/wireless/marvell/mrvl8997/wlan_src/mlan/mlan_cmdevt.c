...
wlan_clean_cmd_node(pmlan_adapter pmadapter, cmd_ctrl_node *pcmd_node)
{
	...
	if (pcmd_node->respbuf) {
		wlan_pcie_cmdrsp_complete(pmadapter, pcmd_node->respbuf);
				...
				t_u32 upld_len)
{
	...
	pmadapter->ps_state = PS_STATE_SLEEP;
	mlan_collect_power_statistic((t_void *)pmadapter);
	LEAVE();
}
...
/**
 *  @brief This function prepares the command to enable/disable DPDLOG EVENT
 *
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param cmd          A pointer to HostCmd_DS_COMMAND structure
 *  @param cmd_action   Command action: GET or SET
 ...
 */
mlan_status
wlan_cmd_dpdlog_enable(IN pmlan_private pmpriv,
		       IN HostCmd_DS_COMMAND *cmd,
		       IN t_u16 cmd_action, IN t_void *pdata_buf)
{
	HostCmd_DS_DPD_LOG_EN *pdpdlog_en = &cmd->params.dpdlog_en;
	ENTER();
	cmd->command = wlan_cpu_to_le16(HostCmd_CMD_DPD_LOG_EN);
	pdpdlog_en->action = wlan_cpu_to_le16(cmd_action);
	cmd->size = wlan_cpu_to_le16(S_DS_GEN + sizeof(HostCmd_DS_DPD_LOG_EN));
	if (cmd_action == HostCmd_ACT_GEN_SET) {
		pdpdlog_en->enable = *((t_u8 *)pdata_buf);
		PRINTM(MCMND, "Set dpdlog enable: %d\n", pdpdlog_en->enable);
	}
	LEAVE();
	return MLAN_STATUS_SUCCESS;
}
...
/**
 *  @brief This function handles the command response of HostCmd_DS_DPD_LOG_EN
 *
 *  @param pmpriv       A pointer to mlan_private structure
 *  @param resp         A pointer to HostCmd_DS_COMMAND
 *  @param pioctl_buf   A pointer to mlan_ioctl_req structure
 ...
 */
mlan_status
wlan_ret_dpdlog_enable(IN pmlan_private pmpriv,
		       IN HostCmd_DS_COMMAND *resp,
		       IN mlan_ioctl_req *pioctl_buf)
{
	mlan_ds_misc_cfg *pcfg = MNULL;
	HostCmd_DS_DPD_LOG_EN *pdpdlog_en = &resp->params.dpdlog_en;

	ENTER();

	if (pioctl_buf) {
		pcfg = (mlan_ds_misc_cfg *)pioctl_buf->pbuf;
		pcfg->param.dpdlog_en = pdpdlog_en->enable;
	}
	LEAVE();
	return MLAN_STATUS_SUCCESS;
}
 ...
 */
mlan_status
wlan_cmd_get_dpd_log(IN pmlan_private pmpriv, IN HostCmd_DS_COMMAND *cmd)
{
	ENTER();
	cmd->command = wlan_cpu_to_le16(HostCmd_CMD_GET_DPD_LOG);
	cmd->size = wlan_cpu_to_le16(sizeof(HostCmd_DS_GET_DPD_LOG) + S_DS_GEN);
	LEAVE();
	return MLAN_STATUS_SUCCESS;
}
 ...
 */
mlan_status
wlan_ret_get_dpdlog(IN pmlan_private pmpriv,
		    IN HostCmd_DS_COMMAND *resp, IN mlan_ioctl_req *pioctl_buf)
{
	HostCmd_DS_GET_DPD_LOG *pget_dpdlog =
		(HostCmd_DS_GET_DPD_LOG *) & resp->params.get_dpdlog;
	...
	if (pioctl_buf) {
		...
		memcpy(pmpriv->adapter,
		       (t_u8 *)&pget_info->param.get_dpdlog.dpdlog,
		       (t_u8 *)&pget_dpdlog->dpdlog, sizeof(dpdlog_t));
		DBG_HEXDUMP(MCMD_D, "dpdlog",
			    (t_u8 *)&pget_info->param.get_dpdlog.dpdlog,
			    sizeof(dpdlog_t));
	}
	LEAVE();
