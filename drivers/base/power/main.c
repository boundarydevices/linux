...
static int dpm_run_callback(struct device *dev, int (*cb)(struct device *), const char* fname)
{
	...
	error = cb(dev);
	suspend_time_debug_report(fname, dev, starttime);
		 ...
		 pm_message_t state)
{
	...
	case PM_EVENT_SUSPEND:
		error = dpm_run_callback(dev, ops->suspend, "suspend");
		break;
	case PM_EVENT_RESUME:
		error = dpm_run_callback(dev, ops->resume, "resume");
		break;
