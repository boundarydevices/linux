...
static int imx_uart_dma_init(struct imx_port *sport)
{
	...
	/*
	 * Set the src_addr_width/src_maxburst according to the relevant files
	 * such as "i.MX61 SDMA Scripts User Manual.doc".
	 * So does to dst_addr_width/dst_maxburst.
	 */
...
static void imx_disable_dma(struct imx_port *sport)
{
	...
	sport->dma_is_enabled = false;
	sport->dma_is_inited = false;
}
		   ...
		   struct ktermios *old)
{
	...
	if (termios->c_cflag & CRTSCTS) {
		if (sport->have_rtscts) {
			...
			/* check if we can enable the DMA */
			if (sport->enable_dma && !sport->dma_is_inited)
				if (!imx_uart_dma_init(sport))
					sport->dma_is_inited = true;
		} else {
