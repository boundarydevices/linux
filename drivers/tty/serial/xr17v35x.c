...
*	   for			: LINUX 3.x.x (Tested on 3.0, 3.2, 3.5 and 3.6)
*	   date			: January 2013
*	   version		: 1.4
*
...
static void serialxr_shutdown(struct uart_port *port)
{
	...
	up->timer.function = serialxr_timeout;
	if (is_real_interrupt(up->port.irq))
...
static int __init serialxr_init(void)
{
	...
	printk(KERN_INFO "Exar PCIe (XR17V35x) serial driver Revision: 1.4\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Exar PCIe specific serial driver for XR17V35x- Revision: 1.4");
