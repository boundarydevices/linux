...
static int camsv_link_setup(struct media_entity *entity,
			     ...
			     const struct media_pad *remote, u32 flags)
{
	...
	if (!sd)
		return -EINVAL;
	cam = v4l2_get_subdevdata(sd);
	if (!cam)
		return -EINVAL;

	if (flags & MEDIA_LNK_FL_ENABLED) {
		cam->seninf_sd = media_entity_to_v4l2_subdev(remote->entity);
		dev_dbg(cam->dev, "%s: link setup: %s -> %s seninf_pad_idx: %d\n",
			__func__,
			sd->entity.name,
			cam->seninf_sd->entity.name,
			remote->index);
