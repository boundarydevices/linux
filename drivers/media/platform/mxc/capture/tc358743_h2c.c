...
struct tc_data {
	...
	int det_changed;
	struct fsl_mxc_camera_platform_data *camera_plat;
...
static void tc358743_enable_edid(struct sensor_data *sensor)
{
	...
	pr_debug("%s: c7=%02x ca=%02x cb=%02x 43=%02x 4a=%02x\n", __func__,
...
static s32 power_control(struct tc_data *td, int on)
{
	...
	struct fsl_mxc_camera_platform_data *plat = td->camera_plat;
	int i;
	int ret = 0;
			...
			const struct i2c_device_id *id)
{
	...
	sensor->mclk = plat->mclk;		/* 6 - 54 MHz */
	sensor->mclk_source = plat->mclk_source;
	sensor->csi = plat->csi;
	sensor->io_init = plat->io_init;
	...
	sregulator[REGULATOR_IO] = plat->io_regulator;
