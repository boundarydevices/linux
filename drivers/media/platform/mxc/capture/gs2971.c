...
static int gs2971_get_gpios(struct gs2971_priv *gs, struct device *dev)
{
	...
	for (i = 0; i < GPIO_CNT; i++) {
		...
		gd = devm_gpiod_get(dev, gpio_names[i], (i >= GPIO_DVB_ASI) ?
			GPIOD_IN : active ? GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
...
static int ioctl_s_parm(struct v4l2_int_device *s, struct v4l2_streamparm *a)
{
	...
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
		...
		if ((timeperframe->numerator == 0) ||
			...
			pr_warn("   Setting framerate to default (%dfps)!\n",
				   gs2971_framerates[gs2971_default_fps]);
...
static void gs2971_remove(struct spi_device *spi)
{
