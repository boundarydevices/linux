...
static char *regulator_names[] = {
	[DVDDIO_REG] = "DOVDD",
	[DVDD_REG] = "DVDD",
	[AVDD_REG] = "AVDD",
	[PVDD_REG] = "PVDD",
};
static unsigned reg_volt[] = {
...
static int adv7180_regulator_enable(struct adv7180_priv *adv, struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(adv->regulators); i++) {
		...
		if (!IS_ERR(adv->regulators[i])) {
			...
			if (ret) {
				dev_err(dev, "regulator_enable failed for %s\n", p);
				return ret;
			} else {
				dev_dbg(dev, "regulator_enable ok for %s\n", p);
			}
		} else {
			dev_warn(dev, "cannot get regulator for %s\n", p);
		}
	}
	return ret;
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	adv7180_regulator_enable(adv, dev);

	printk(KERN_ERR"DBG adv data is at %p\n", adv);

	msleep(1);
	adv->pwn_gpio = pwn_gpio;
	...
	if (IS_ERR(adv->sen.sensor_clk)) {
		...
		return PTR_ERR(adv->sen.sensor_clk);
	}
	...
	dev_dbg(dev, "%s:adv7180 probe i2c address is 0x%02X\n",
		__func__, adv->sen.i2c_client->addr);

	/*! Read the revision ID of the tvin chip */
	rev_id = adv7180_read(adv, ADV7180_IDENT);
	dev_dbg(dev, "%s:Analog Device adv7%2X0 detected!\n", __func__, rev_id);
