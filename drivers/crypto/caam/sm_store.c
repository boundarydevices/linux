...
int caam_sm_startup(struct platform_device *pdev)
{
	...
	/*
	 * A virtual address for secure memory exists in the driver
	 * private block (and was mapped in the caam controller driver;
	 * AKA ctrl.c). Since it's not a memory region "known" to the
	 * kernel, computing a physical address from virtual isn't so
	 * simple. So, check the identifying resource, and use the
	...
	for (page = 0; page < smpriv->max_pages; page++) {
		...
		if (((pgstat & SMCS_PGWON_MASK) >> SMCS_PGOWN_SHIFT)
			...
			lpagedesc[page].pg_phys = (u32 *)res->start +
				((smpriv->page_size * page) / sizeof(u32));
			lpagect++;
#ifdef SM_DEBUG
			dev_info(smdev,
