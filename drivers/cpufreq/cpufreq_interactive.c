...
{
	int i, j = 0;
	j += scnprintf(&buf[j], CPUFREQ_NOTE_LEN, "Change irq setting \
		by echo a data, format: 0xAABBBC, AA:irq number,\
		BBB:up_threshold, C:enable\n");
	for (i = 0; i < MAX_CPUFREQ_IRQ_NUMBER; i++) {
		if (irq_tuner_ins[i].irq_number != 0)
			j += scnprintf(&buf[j], CPUFREQ_IRQ_LEN, "irq\
			number: %d\tup_threshold %d\t%s\n",
			irq_tuner_ins[i].irq_number,
			irq_tuner_ins[i].up_threshold,
			irq_tuner_ins[i].enable ? "enabled" : "disabled");
	}
...
int cpufreq_gov_irq_tuner_register(void)
{
	int ret = 0, i;
	...
	np = of_find_node_by_name(NULL, "irq_tuner");
	if (!np) {
	...
	i = 0;
	for_each_compatible_node(np, NULL, "irq-threshold") {
		if (of_property_read_u32(np, "irq_number",
			&(irq_tuner_ins[i].irq_number)))
			continue;
		if (of_property_read_u32(np, "up_threshold",
			&(irq_tuner_ins[i].up_threshold)))
			continue;
		irq_tuner_ins[i].enable = of_property_read_bool(np, "enabled");
		cpufreq_irq_desc[i] = irq_to_desc(irq_tuner_ins[i].irq_number);
		i++;
		if (i >= MAX_CPUFREQ_IRQ_NUMBER)
			printk(KERN_WARNING " %s too many irq numbers!\n",
				__func__);
	}
...
static int __init cpufreq_interactive_init(void)
{
	...
	cpufreq_gov_irq_tuner_register();
