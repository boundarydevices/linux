...
static irqreturn_t max11801_ts_interrupt(int irq, void *dev_id)
{
	...
	if (max11801_workmode == 0) {
		if (status & (MAX11801_FIFO_INT | MAX11801_FIFO_OVERFLOW)) {
			status = read_register(data->client,
			GENERNAL_STATUS_REG);
			...
			if ((buf[1] & EVENT_TAG_MASK) !=
				(buf[3] & EVENT_TAG_MASK))
				goto out;
...
return IRQ_HANDLED;
		...
		if (status & (MAX11801_EDGE_INT)) {
			...
			FIFO_RD_X_MSB, 1, temp_buf);
			x_buf[0] = temp_buf[0];
			if (ret < 1)
				goto out2;
			ret = i2c_smbus_read_i2c_block_data(client,
			FIFO_RD_X_LSB, 1, temp_buf);
			x_buf[1] = temp_buf[0];
			if (ret < 1)
				goto out2;
			/* Y = panel setup*/
			max11801_dcm_write_command(client,
			Panel_Setup_Y);
			/* Y_measurement*/
			max11801_dcm_write_command(client,
			Y_measurement);
			ret = i2c_smbus_read_i2c_block_data(client,
			FIFO_RD_Y_MSB, 1, temp_buf);
			y_buf[0] = temp_buf[0];
			if (ret < 1)
				goto out2;
			ret = i2c_smbus_read_i2c_block_data(client,
			FIFO_RD_Y_LSB, 1, temp_buf);
			y_buf[1] = temp_buf[0];
			if (ret < 1)
				goto out2;

			if ((x_buf[1] & MEASURE_TAG_MASK)
				== MEASURE_X_TAG)
				x = (x_buf[0] << XY_BUF_OFFSET) +
					(x_buf[1] >> XY_BUF_OFFSET);
			if ((y_buf[1] & MEASURE_TAG_MASK)
				== MEASURE_Y_TAG)
				y = (y_buf[0] << XY_BUF_OFFSET) +
					(y_buf[1] >> XY_BUF_OFFSET);

			if ((x_buf[1] & EVENT_TAG_MASK) !=
				(y_buf[1] & EVENT_TAG_MASK))
				goto out2;
...
static void __devinit max11801_ts_phy_init(struct max11801_data *data)
{
	...
	if (max11801_workmode == 0) {
		...
		* average eight media sample
		*/
		max11801_write_reg(client, MESURE_AVER_CONF_REG, 0xff);
		/*
		* X,Y panel setup time set
		* to 20us
		*/
		max11801_write_reg(client, PANEL_SETUPTIME_CONF_REG, 0x11);
		/*
		* Rough pullup time (2uS),
		* Fine pullup time (10us)
		*/
		max11801_write_reg(client, TOUCH_DETECT_PULLUP_CONF_REG, 0x10);
		/*
		* Auto mode init period = 5ms,
		* scan period = 5ms
		*/
	...
	if (max11801_workmode == 1) {
		...
		* enable Aperture, enable Average X,Y
		*/
				       ...
				       const struct i2c_device_id *id)
{
	...
	gpio = of_get_named_gpio(of_node, "int_input", 0);
	if (!gpio_is_valid(gpio)) {
		dev_err(&client->dev, "Failed to get dts node\n");
		error = -EINVAL;
		goto err_free_mem;
	}

	error = gpio_request_one(gpio, GPIOF_IN, "max11801-int");
	if (error) {
		printk(KERN_ERR"request max11801-int error!!\n");
		goto err_free_mem;
	}
	...
	error = request_threaded_irq(gpio_to_irq(gpio), NULL,
			max11801_ts_interrupt,
			 ...
			 "max11801_ts", data);
	if (error) {
		dev_err(&client->dev, "Failed to request_irq\n");
		if (gpio)
			gpio_free(gpio);
		goto err_free_mem;
		}
