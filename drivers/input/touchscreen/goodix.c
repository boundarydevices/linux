...
static int goodix_configure_dev(struct goodix_ts_data *ts)
{
	...
	ts->swapped_x_y = of_property_read_bool(ts->client->dev.of_node,
						    "touchscreen-swapped-x-y");
	ts->inverted_x = of_property_read_bool(ts->client->dev.of_node,
						   "touchscreen-inverted-x");
	ts->inverted_y = of_property_read_bool(ts->client->dev.of_node,
