...
static int ili_spi_wrapper(u8 *txbuf, u32 wlen, u8 *rxbuf, u32 rlen, bool spi_irq, bool i2c_irq)
{
	...
	case SPI_WRITE:
		...
		if (ice) {
			...
			if (ilits->spi_ms_mode == SLAVE)
				wdata[0] = SPI_WRITE_SLAVE0;
			else
				wdata[0] = SPI_WRITE;
#else
			wdata[0] = SPI_WRITE;
...
static int ilitek_spi_probe(struct spi_device *spi)
{
...
#if SUSPEND_RESUME_SUPPORT
#if QCOM_PANEL_SUSPEND_RESUME | (defined(__DRM_PANEL_H__) && defined(DRM_PANEL_EARLY_EVENT_BLANK))
	ret = ili_v3_drm_check_dt(spi->dev.of_node);
	if (ret < 0) {
	...
	if (spi->controller->flags & SPI_CONTROLLER_HALF_DUPLEX) {
		ILI_ERR("Full duplex not supported by controller\n");
		return -EIO;
	}
#endif
