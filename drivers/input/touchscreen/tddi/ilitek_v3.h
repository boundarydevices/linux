...
#define DRIVER_VERSION			"3.0.12.0.241001"
...
#define TR_BUF_SIZE					(10*K)	/* Buffer size of touch report */
...
#define ENABLE_GET_ALL_INFO 		DISABLE /*GET_ALL_INFO options*/
#define ENABLE_PEN_MODE 			DISABLE	/*Pen mode options*/
...
#define MTK_DISP_SUSPEND_RESUME		DISABLE
#define QCOM_PANEL_SUSPEND_RESUME	DISABLE

#if QCOM_PANEL_SUSPEND_RESUME
...
#define INTR33_reg_dma_ch1_int_en			BIT(17)
...
#define P5_X_DEBUG_LITE_A9_PACKET_ID        0xA9
...
#define ILI79451						0x794501
#define ILI79900						0x799000
...
#define SRAM_TEST_SETTING_LEN     			16
...
struct sram_test_para {
	...
	bool sram_group_en;
	u8 group_num;
	u8 write_setting_num;
	u8 read_setting_num;
	u32 MaxAddr;
	u32 MinAddr;
	u32 write_addr[SRAM_TEST_SETTING_LEN];
	u32 write_addr_len[SRAM_TEST_SETTING_LEN];
	u32 read_addr[SRAM_TEST_SETTING_LEN];
	char write_info[SRAM_TEST_GROUP_LEN][SRAM_TEST_GROUP_STR_LEN];
	char read_info[SRAM_TEST_GROUP_LEN][SRAM_TEST_GROUP_STR_LEN];
	u32 SramPartialAreaTest_en[SRAM_TEST_GROUP_LEN][SRAM_TEST_SETTING_LEN];
	u32 SramResult[SRAM_TEST_GROUP_LEN][SRAM_TEST_SETTING_LEN];
...
extern void ili_flash_dma_write_cascade(u32 start, u32 end, u32 len);
extern void ili_flash_clear_dma(void);
extern void ili_flash_clear_dma_cascade(void);
extern void ili_fw_read_flash_info(bool mcu);
extern int ilitek_tddi_flash_protect(bool enable, bool mcu);
...
extern void ili_report_separating_finger_pen_ap_mode(u8 *buf, int len);
...
extern void ili_ic_get_report_info(void);
...
extern int ili_ice_mode_bit_mask_write_cascade(u32 addr, u32 mask, u32 value, int mode);
...
extern int ili_v3_drm_check_dt(struct device_node *np);
#endif
#endif
...
extern int ili_ice_mode_ctrl_by_mode_spi(bool enable, bool mcu, int mode);
extern int ili_ice_mode_ctrl_by_mode_i2c(bool enable, bool mcu, int mode);
extern int ili_spi_ice_mode_read(u32 addr, u32 *data, int len, u8 msmode);
extern int ili_ice_slave_write_register(u32 addr, u32 data, int len);
extern void get_mp_info_log(char *input, int len, int err_code);
