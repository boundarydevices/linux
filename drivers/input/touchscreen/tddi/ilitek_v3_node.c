...
#define ILITEK_IOCTL_SWITCH_ICE_FLASH_CS_CTL		_IOWR(ILITEK_IOCTL_MAGIC, 40, u8*)
...
int ili_cascade_rw_tp_reg(u32 mcu, u32 type, u32 addr, u32 wdata, u32 wlen, u32 *rdata)
{
	...
	if (ilits->slave_wr_ctrl == SLAVE) {
		...
		ret = ili_ice_mode_ctrl_by_mode(ENABLE, (mcu == mcu_on) ? ON : OFF, MASTER);
		if (ret < 0) {
	...
	if (type == read) {
		ili_spi_ice_mode_read(addr, &read_data, sizeof(u32), ilits->slave_wr_ctrl);
		*rdata = read_data;
	...
	if (ilits->slave_wr_ctrl == SLAVE)
		...
		ret = ili_ice_mode_ctrl(DISABLE, (mcu == mcu_on) ? ON : OFF );
...
{
	int len = 0;
	ILI_DBG("type = %d, slave_wr_ctrl = %d\n", type, ilits->slave_wr_ctrl);
	if (type == ILI_WRITE && ilits->slave_wr_ctrl == SLAVE) {
		ili_ice_slave_write_register(addr, wdata, wlen);
		ILI_INFO("[WRITE]:addr = 0x%06x, write = 0x%08x, len = %d byte\n", addr, wdata, wlen);
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "WRITE:addr = 0x%06x, write = 0x%08x, len =%d byte\n", addr, wdata, wlen);
	} else {
		len = ili_cascade_rw_tp_reg(mcu, type, addr, wdata, wlen, rdata);
	}
...
#if (TDDI_INTERFACE == BUS_SPI)
#if ENABLE_SPICASCADE_V2
		len = ili_cascade_rw_tp_reg_v2(stop_mcu, type, addr, write_data, write_len, &read_data);
#else
...
static ssize_t ilitek_proc_fw_cmd_read(struct file *pFile, char __user *buf, size_t size, loff_t *pos)
{
	...
	if (wrap_wlen + 4 > sizeof(fw_cmd_buff)) {
		ILI_ERR("wrap_wlen exceeds buffer size\n");
	}else{
		...
		if (ilits->dbf_out[count].mark) {
			if (ilits->dbf_out[count].data[0] == P5_X_DEMO_PACKET_ID) {
				...
				need_read_data_len = data_count * one_data_bytes + 1 + 5;
					...
					|| ilits->dbf_out[count].data[0] == P5_X_DEBUG_LITE_A9_PACKET_ID			/* A9 */
					|| ilits->dbf_out[count].data[0] == P5_X_DEMO_HIGH_RESOLUTION_PACKET_ID		/* 5B */
					|| ilits->dbf_out[count].data[0] == P5_X_DEMO_FINGER_PACKET_ID				/* 81 */
					|| ilits->dbf_out[count].data[0] == P5_X_DEMO_PEN_PACKET_ID					/* 82 */
	...
	if (ilits->dbl[ilits->odi].mark) {
		if (ilits->dbl[ilits->odi].data[0] == P5_X_DEMO_PACKET_ID) {
			...
			need_read_data_len = data_count * one_data_bytes + 1 + 5;
				...
				|| ilits->dbl[ilits->odi].data[0] == P5_X_DEBUG_LITE_A9_PACKET_ID	/* A9 */
				|| ilits->dbl[ilits->odi].data[0] == P5_X_DEMO_HIGH_RESOLUTION_PACKET_ID	/* 5B */
				|| ilits->dbl[ilits->odi].data[0] == P5_X_DEMO_FINGER_PACKET_ID				/* 81 */
				|| ilits->dbl[ilits->odi].data[0] == P5_X_DEMO_PEN_PACKET_ID				/* 82 */
...
static ssize_t ilitek_proc_debug_message_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	if (ilits->dbl[ilits->odi].mark) {
		...
		if (need_read_data_len > 2040) {
			need_read_data_len = 2040;
			ILI_INFO("Need Read data len = %d\n", need_read_data_len);
		}
...
static ssize_t ilitek_node_mp_lcm_on_test_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	get_mp_info_log(mp_log_info, sizeof(mp_log_info)/sizeof(char), ret);
...
static ssize_t ilitek_node_ver_info_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Firmware AP Version = %d.%d.%d.%d\n",
			ilits->chip->fw_ver >> 24, (ilits->chip->fw_ver >> 16) & 0xFF,
			(ilits->chip->fw_ver >> 8) & 0xFF, ilits->chip->fw_ver & 0xFF);
	len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "Firmware MP Version = %d.%d.%d.%d\n",
...
static ssize_t ilitek_node_fw_upgrade_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	...
	} else if (ret == EFW_BACKUP) {
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to backup chunk!");
	} else if (ret == EFW_FP) {
		len += snprintf(g_user_buf + len, USER_STR_BUFF - len, "%s\n", "Failed to flash protect, abort!");
	}
...
static int ili_tp_data_mode_ctrl(u8 *cmd)
{
	...
	case DEBUG_LITE_PEN:
		...
		break;
	case DEBUG_LITE_PEN_AREA:
		if (ilits->PenType == POSITION_PEN_TYPE_ON) {
			if (ili_set_tp_data_len(DATA_FORMAT_DEBUG_LITE_PEN_AREA, false, &cmd[1]) < 0) {
				ILI_ERR("Failed to set debug lite pen\n");
				ret = -ENOTTY;
			}
		}
	...
	case DEBUG_LITE_PEN:
		...
		break;
	case DEBUG_LITE_PEN_AREA:
		ILI_INFO("SPI no support debug lite pen area\n");
		break;
#endif
...
static int ili_mp_lcm_off_env_ctrl(u8 lcm_on)
{
	int ret = 0;
...
static long ilitek_node_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	bool read = 0;
	u32 type, addr, read_data, write_data;
	int write_len = 0;
	...
	case ILITEK_IOCTL_TP_FW_VER:
		ILI_DBG("ioctl: get Firmware AP Version and Firmware MP Version\n");
		...
		ILI_DBG("Firmware AP Version = %d.%d.%d.%d\n", szBuf[0], szBuf[1], szBuf[2], szBuf[3]);
	...
	case ILITEK_IOCTL_ICE_MODE_SWITCH:
		...
		ILI_DBG("ioctl: switch ice mode = %d, mcu on = %d, change key = %d, skip sync cmd = %d\n", szBuf[0], szBuf[1], szBuf[3], ilits->skip_sync_cmd);
		ilits->change_ice_key = szBuf[3] == 1;
	...
	case ILITEK_IOCTL_SLAVE_CTRL:
		...
		ilits->spi_ms_mode = szBuf[0];
		ILI_DBG(" spi ms mode = %d\n", ilits->spi_ms_mode);
	...
	case ILITEK_IOCTL_CASCADE_REG_RW:
		...
		if (type == read) {
			...
			if (ilits->slave_wr_ctrl == SLAVE) {
#if ENABLE_SPICASCADE_V2
				ili_ice_slave_write_register(addr, write_data, write_len);
#else
	...
	case ILITEK_IOCTL_ICE_MODE_FLAG_SET:
		...
		break;
	case ILITEK_IOCTL_SWITCH_SYNC_CTL:
		if (copy_from_user(szBuf, (u8 *) arg, 1)) {
			ILI_ERR("Failed to copy data from user space\n");
			ret = -ENOTTY;
			break;
		...
		ILI_DBG("ioctl: set sync ctl = %d\n", szBuf[0]);

		if (szBuf[0] == 0) {
			ilits->skip_sync_cmd = true;
		} else {
			ilits->skip_sync_cmd = false;
		}
		break;
	case ILITEK_IOCTL_SWITCH_ICE_FLASH_CS_CTL:
		if (copy_from_user(szBuf, (u8 *) arg, 1)) {
			ILI_ERR("Failed to copy data from user space\n");
			ret = -ENOTTY;
			break;
		...
		ILI_DBG("ioctl: set flash cs ctl = %d\n", szBuf[0]);
