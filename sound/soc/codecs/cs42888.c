	...
	const struct i2c_device_id *id)
{
	...
	int val;
	u32 supported_rates;
	const struct device_node *codec_np = i2c_client->dev.of_node;
	...
	ret = of_property_read_u32(codec_np, "rates", &supported_rates);
	if (ret) {
		printk(KERN_ERR "%s: failed to get rates!\n", __func__);
		return -EINVAL;
	}

	cs42888_dai.playback.rates = supported_rates;
	cs42888_dai.capture.rates = supported_rates;
