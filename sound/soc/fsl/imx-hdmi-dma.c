...
#define hdmi_audio_debug(reg) \
	pr_debug(#reg "\r\t\t\t\t0x%02x\n", hdmi_readb(reg))

#ifdef DEBUG
static void dumpregs(void)
{
	pr_debug("\n");
	hdmi_audio_debug(HDMI_AHB_DMA_CONF0);
	hdmi_audio_debug(HDMI_AHB_DMA_START);
	hdmi_audio_debug(HDMI_AHB_DMA_STOP);
	hdmi_audio_debug(HDMI_AHB_DMA_THRSLD);
	hdmi_audio_debug(HDMI_AHB_DMA_STRADDR0);
	hdmi_audio_debug(HDMI_AHB_DMA_STPADDR0);
	hdmi_audio_debug(HDMI_AHB_DMA_BSTADDR0);
	hdmi_audio_debug(HDMI_AHB_DMA_MBLENGTH0);
	hdmi_audio_debug(HDMI_AHB_DMA_MBLENGTH1);
	hdmi_audio_debug(HDMI_AHB_DMA_STAT);
	hdmi_audio_debug(HDMI_AHB_DMA_INT);
	hdmi_audio_debug(HDMI_AHB_DMA_MASK);
	hdmi_audio_debug(HDMI_AHB_DMA_POL);
	hdmi_audio_debug(HDMI_AHB_DMA_CONF1);
	hdmi_audio_debug(HDMI_AHB_DMA_BUFFSTAT);
	hdmi_audio_debug(HDMI_AHB_DMA_BUFFINT);
	hdmi_audio_debug(HDMI_AHB_DMA_BUFFMASK);
	hdmi_audio_debug(HDMI_AHB_DMA_BUFFPOL);
	hdmi_audio_debug(HDMI_IH_MUTE_AHBDMAAUD_STAT0);
	hdmi_audio_debug(HDMI_IH_AHBDMAAUD_STAT0);
	hdmi_audio_debug(HDMI_IH_MUTE);
	pr_debug("\n");
}
...
static void hdmi_dma_irq_mask(int mask)
{
	...
	if (mask)
		regval |= HDMI_AHB_DMA_DONE;
	else
		regval &= (u8)~HDMI_AHB_DMA_DONE;
...
{
	hdmi_writeb(mute ? 0xff : 0x0, HDMI_IH_MUTE_AHBDMAAUD_STAT0);
}
...
#ifndef HDMI_DMA_USE_NEON
/* C code optimization for IEC head */
static void hdmi_dma_copy_16_c_lut(u16 *src, u32 *dst,
		int samples, u8 *lookup_table)
{
	u32 sample, head, p;
	int i;
...
static void hdmi_dma_copy_16(u16 *src, u32 *dst,
		...
		int framecount, int channelcount)
{
	...
	for (i = 0; i < count_in_192; i++) {
		...
		count = (framecount < 48) ? framecount : 48;
		samples = count * channelcount;
		hdmi_dma_copy_16_neon_lut(src, dst,
				samples, g_packet_head_table);
		framecount -= count;
		if (framecount == 0)
			break;
...
* Sample count (frame_count * channel_count) is multipliable by 8.
*/
static void hdmi_dma_copy_24(u32 *src, u32 *dst,
		int framecount, int channelcount)
{
	/* split input frames into 192-frame each */
	int count_in_192 = (framecount + 191) / 192;
	...
	for (i = 0; i < count_in_192; i++) {
		...
		count = (framecount < 48) ? framecount : 48;
		samples = count * channelcount;
		hdmi_dma_copy_24_neon_lut(src, dst,
				samples, g_packet_head_table);
		framecount -= count;
		if (framecount == 0)
			break;
		...
		count = (framecount < 192 - 48) ? framecount : 192 - 48;
		samples = count * channelcount;
		hdmi_dma_copy_24_neon_fast(src, dst, samples);
		framecount -= count;
		src  += samples;
		dst += samples;
	}
		...
		int offset, int count)
{
	...
	dst = (u32 *)(rtd->hw_buffer.area + (offset * rtd->buffer_ratio));

	switch (rtd->format) {
	case SNDRV_PCM_FORMAT_S16_LE:
		/* dma_buffer is the mmapped buffer we are copying pcm from. */
		src16 = (u16 *)(runtime->dma_area + offset);
		hdmi_dma_copy_16(src16, dst, framecount, rtd->channels);
		break;
	default:
...
{
	u8 val = hdmi_readb(HDMI_AHB_DMA_CONF0) &
...
static void hdmi_dma_enable_channels(int channels)
{
	...
	case 8:
		...
		break;
	default:
		pr_err("%s - invalid audio channels number: %d\n",
				__func__, channels);
		break;
	}
}
...
static void hdmi_dma_set_thrsld_incrtype(int channels)
{
	...
	case 0x1a:
		...
		break;
	default:
		pr_err("ERROR: unrecognized hdmi controller!\r\n");
		break;
	}
...
static int hdmi_dma_copy(struct snd_pcm_substream *substream, int channel,
		snd_pcm_uframes_t pos, void __user *buf, snd_pcm_uframes_t frames)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct imx_hdmi_dma_runtime_data *rtd = runtime->private_data;
...
{
	struct hdmi_sdma_script_data *hdmi_sdma_t = params->hdmi_sdma_t;
	...
	tmp_addr1 = (u32 *)hdmi_sdma_t + 3;
	tmp_addr2 = (u32 *)hdmi_sdma_t + 4;
...
static int hdmi_sdma_alloc(struct snd_pcm_substream *substream,
		struct imx_hdmi_dma_runtime_data *params)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct imx_hdmi_sdma_params *dma_params;
	...
	params->dma_data.dma_request = dma_params->dma;

	if (params->dma_data.dma_request == -1) {
		dev_err(&params->dma_channel->dev->device,
			"Invalid dma request.\n");
		return -EINVAL;
	}
	...
	params->dma_channel =
		dma_request_channel(mask, hdmi_filter, &params->dma_data);
...
static int hdmi_dma_hw_free(struct snd_pcm_substream *substream)
{
	...
	if (use_sdma && params->dma_channel) {
				...
				struct snd_pcm_hw_params *params)
{
	...
	rtd->offset = 0;
	rtd->period_time = HZ / (rtd->rate / params_period_size(params));
	...
	if (use_sdma) {
		...
		ret = hdmi_init_sdma_buffer(rtd);
		if (ret)
			return ret;

		ret = hdmi_sdma_alloc(substream, rtd);
		if (ret)
			return ret;

		ret = hdmi_sdma_config(rtd);
		if (ret)
			return ret;
	}
...
static int hdmi_dma_trigger(struct snd_pcm_substream *substream, int cmd)
{
	...
	case SNDRV_PCM_TRIGGER_PAUSE_RELEASE:
		...
		if (use_sdma) {
			...
			dma_async_issue_pending(rtd->desc->chan);
		}
		break;
...
static void hdmi_dma_irq_enable(struct imx_hdmi_dma_runtime_data *rtd)
{
	...
	hdmi_dma_irq_mute(use_sdma ? 1 : 0);
	hdmi_dma_irq_mask(0);
	hdmi_mask(0);
...
static int imx_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
	...
	buf->area = dma_alloc_coherent(pcm->card->dev,
			HDMI_PCM_BUF_SIZE, &buf->addr, GFP_KERNEL);
	if (!buf->area) {
		pr_err("%s: Failed to alloc bef->area\n", __func__);
		return -ENOMEM;
	}
	...
	hw_buffer->area = dma_alloc_coherent(pcm->card->dev,
			HDMI_DMA_BUF_SIZE, &hw_buffer->addr, GFP_KERNEL);
	if (!hw_buffer->area) {
		pr_err("%s: Failed to alloc hw_buffer->area\n", __func__);
		return -ENOMEM;
	}
...
static int imx_hdmi_dma_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	...
	ret = imx_pcm_preallocate_dma_buffer(pcm, SNDRV_PCM_STREAM_PLAYBACK);
	if (ret)
		pr_err("%s: Failed to preallocate dma buffer", __func__);
...
static int __devinit imx_soc_platform_probe(struct platform_device *pdev)
{
	...
	hdmi_dma_priv = devm_kzalloc(&pdev->dev,
			sizeof(*hdmi_dma_priv), GFP_KERNEL);
	if (!hdmi_dma_priv) {
		dev_err(&pdev->dev, "Failed to alloc hdmi_dma_priv\n");
		return -ENOMEM;
	}

	use_sdma = (hdmi_SDMA_check()) ? true : false;
	...
	if (use_sdma) {
		...
		hdmi_dma_priv->hdmi_sdma_t =
			dma_alloc_noncacheable(NULL,
				...
				&hdmi_dma_priv->phy_hdmi_sdma_t, GFP_KERNEL);
		if (!hdmi_dma_priv->hdmi_sdma_t) {
...
e_irq:
	if (use_sdma)
		dma_free_coherent(NULL, sizeof(struct hdmi_sdma_script_data),
				hdmi_dma_priv->hdmi_sdma_t,
