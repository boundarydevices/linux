...
static int mxc_hdmi_update_constraints(struct snd_pcm_substream *substream)
{
...
static int mxc_hdmi_soc_startup(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct imx_hdmi *hdmi_data = snd_soc_dai_get_drvdata(dai);
	int ret;

	clk_enable(hdmi_data->isfr_clk);
	clk_enable(hdmi_data->iahb_clk);

	dev_dbg(dai->dev, "%s hdmi clks: isfr:%d iahb:%d\n", __func__,
			(int)clk_get_rate(hdmi_data->isfr_clk),
			(int)clk_get_rate(hdmi_data->iahb_clk));

	snd_soc_dai_set_dma_data(dai, substream, &hdmi_data->dma_params_tx);

	ret = mxc_hdmi_update_constraints(substream);
	if (ret < 0)
		return ret;
...
static void mxc_hdmi_soc_shutdown(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct imx_hdmi *hdmi_data = snd_soc_dai_get_drvdata(dai);

	clk_disable(hdmi_data->iahb_clk);
	clk_disable(hdmi_data->isfr_clk);
}

static int mxc_hdmi_soc_prepare(struct snd_pcm_substream *substream,
		struct snd_soc_dai *dai)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
...
static struct snd_soc_dai_ops mxc_hdmi_soc_dai_ops = {
	.startup = mxc_hdmi_soc_startup,
	.shutdown = mxc_hdmi_soc_shutdown,
	.prepare = mxc_hdmi_soc_prepare,
};

static int mxc_hdmi_soc_dai_probe(struct snd_soc_dai *dai)
{
	int ret;

	init_channel_allocations();

	ret = snd_soc_add_dai_controls(dai, mxc_hdmi_ctrls,
			ARRAY_SIZE(mxc_hdmi_ctrls));
	if (ret)
		dev_err(dai->dev, "Failed to add dai controls\n");

	/* dai_controls won't break the main function, thus normal return. */
	return 0;
}
...
static struct snd_soc_dai_driver fsl_hdmi_dai = {
	...
	.probe = &mxc_hdmi_soc_dai_probe,
	.ops = &mxc_hdmi_soc_dai_ops,
};
...
static int fsl_hdmi_dai_probe(struct platform_device *pdev)
{
	...
	hdmi_data->isfr_clk = clk_get(&pdev->dev, "hdmi_isfr");
	...
	hdmi_data->iahb_clk = clk_get(&pdev->dev, "hdmi_iahb");
	if (IS_ERR(hdmi_data->iahb_clk)) {
		ret = PTR_ERR(hdmi_data->iahb_clk);
		dev_err(&pdev->dev, "Unable to get HDMI ahb clk: %d\n", ret);
		goto e_iahb_clk;
	}
	clk_prepare(hdmi_data->isfr_clk);
	clk_prepare(hdmi_data->iahb_clk);

	ret = of_property_read_u32(pdev->dev.of_node,
			"fsl,hdmi-dma-events", &dma_event);
	if (ret) {
		dev_err(&pdev->dev, "could not get dma events\n");
		dma_event = -1;
	}
	hdmi_data->dma_params_tx.dma = dma_event;

	ret = snd_soc_register_dai(&pdev->dev, &fsl_hdmi_dai);
	if (ret) {
...
e_reg_dai:
	clk_put(hdmi_data->iahb_clk);
e_iahb_clk:
	clk_put(hdmi_data->isfr_clk);
	return ret;
}

static int __devexit fsl_hdmi_dai_remove(struct platform_device *pdev)
{
	struct imx_hdmi *hdmi_data = platform_get_drvdata(pdev);

	clk_unprepare(hdmi_data->iahb_clk);
	clk_unprepare(hdmi_data->isfr_clk);
	clk_put(hdmi_data->iahb_clk);
	clk_put(hdmi_data->isfr_clk);
...
static struct platform_driver fsl_hdmi_driver = {
	...
	.remove = __devexit_p(fsl_hdmi_dai_remove),
	.driver = {
