/*
 * mt8365-sb35.c  --  MT8365 SB35 machine driver
 *
...
static const char * const mt8365_sb35_pin_str[PIN_STATE_MAX] = {
	"aud_mosi_on",
	"aud_mosi_off",
	"aud_miso_on",
	"aud_miso_off",
...
struct mt8365_sb35_dmic_ctrl_data {
...
static int mt8365_sb35_int_adda_startup(struct snd_pcm_substream *substream)
{
...
static void mt8365_sb35_int_adda_shutdown(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct mt8365_sb35_priv *priv = snd_soc_card_get_drvdata(rtd->card);
	int ret = 0;
...
static struct snd_soc_ops mt8365_sb35_int_adda_ops = {
	.startup = mt8365_sb35_int_adda_startup,
	.shutdown = mt8365_sb35_int_adda_shutdown,
};
...
static struct snd_soc_dai_link mt8365_sb35_dais[] = {
	/* Front End DAI links */
...
static int mt8365_sb35_gpio_probe(struct snd_soc_card *card)
{
	...
	for (i = 0 ; i < PIN_STATE_MAX ; i++) {
		priv->pin_states[i] = pinctrl_lookup_state(priv->pinctrl,
			mt8365_sb35_pin_str[i]);
		if (IS_ERR(priv->pin_states[i])) {
			ret = PTR_ERR(priv->pin_states[i]);
			dev_info(card->dev, "%s Can't find pin state %s %d\n",
				 __func__, mt8365_sb35_pin_str[i], ret);
		}
	}
	...
	for (i = PIN_STATE_DEFAULT ; i < PIN_STATE_MAX ; i++) {
		if (IS_ERR(priv->pin_states[i])) {
			dev_info(card->dev, "%s check pin %s state err\n",
				 __func__, mt8365_sb35_pin_str[i]);
			continue;
		}
...
static void mt8365_sb35_parse_of(struct snd_soc_card *card,
...
static struct snd_soc_card mt8365_sb35_card = {
	.name = "mt-snd-card",
	.owner = THIS_MODULE,
	.dai_link = mt8365_sb35_dais,
	.num_links = ARRAY_SIZE(mt8365_sb35_dais),
	.dapm_widgets = mt8365_sb35_widgets,
	.num_dapm_widgets = ARRAY_SIZE(mt8365_sb35_widgets),
	.dapm_routes = mt8365_sb35_routes,
	.num_dapm_routes = ARRAY_SIZE(mt8365_sb35_routes),
};

static int mt8365_sb35_dev_probe(struct platform_device *pdev)
{
	struct snd_soc_card *card = &mt8365_sb35_card;
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct device_node *platform_node;
	struct mt8365_sb35_priv *priv;
	int i, ret;
	...
	for (i = 0; i < card->num_links; i++) {
		if (mt8365_sb35_dais[i].platforms->name)
			continue;
		mt8365_sb35_dais[i].platforms->of_node = platform_node;
	}

	card->dev = dev;

	priv = devm_kzalloc(dev, sizeof(struct mt8365_sb35_priv),
			    GFP_KERNEL);
	if (!priv) {
		ret = -ENOMEM;
	...
	mt8365_sb35_gpio_probe(card);

	mt8365_sb35_parse_of(card, np);
...
static const struct of_device_id mt8365_sb35_dt_match[] = {
	{ .compatible = "mediatek,mt8365-sb35", },
	{ }
};
MODULE_DEVICE_TABLE(of, mt8365_sb35_dt_match);

static struct platform_driver mt8365_sb35_driver = {
	.driver = {
		   .name = "mt8365-sb35",
		   .of_match_table = mt8365_sb35_dt_match,
#ifdef CONFIG_PM
		   .pm = &snd_soc_pm_ops,
#endif
	},
	.probe = mt8365_sb35_dev_probe,
};

module_platform_driver(mt8365_sb35_driver);

/* Module information */
MODULE_DESCRIPTION("MT8365 SB35 SoC machine driver");
MODULE_AUTHOR("Nicolas Belin <nbelin@baylibre.com>");
MODULE_LICENSE("GPL v2");
MODULE_ALIAS("platform:mt8365-sb35");
