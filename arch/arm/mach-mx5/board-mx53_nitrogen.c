...
static int n53_sata_init(struct device *dev, void __iomem *addr)
{
	u32 tmpdata;
	int ret = 0;
	struct clk *clk;
...
static void n53_sata_exit(struct device *dev)
{
	clk_disable(sata_ref_clk);
	clk_put(sata_ref_clk);
...
#if defined(CONFIG_MACH_MX53_NITROGEN_A)
	{
		.type	= EV_KEY,
		.gpio	= MAKE_GP(3,22),
...
static struct fsl_mxc_camera_platform_data hdmi_camera_data = {
	.csi = 0,
	.sensor_name = "tfp410",
};
	...
	unsigned da9052_irq, struct imxi2c_platform_data *i2c2_data)
{
	...
	imx53_add_imx_i2c(0, &mxci2c0_data);
	imx53_add_imx_i2c(1, &mxci2c1_data);
	imx53_add_imx_i2c(2, i2c2_data);
	imx53_add_srtc();
	...
	/* this call required to release SCC RAM partition held by ROM
	  ...
	  */
	imx53_add_mxc_scc2();
        mxc_register_device(&mxc_pm_device, &nitrogen_pm_data);
	imx53_add_dvfs_core(&dvfs_core_data);
	imx53_add_busfreq();

	imx53_add_iim(&iim_data);
	imx53_add_mxc_pwm(0, &mxc_pwm1_platform_data);
	imx53_add_mxc_pwm(1, &mxc_pwm2_platform_data);
	imx53_add_mxc_pwm_backlight(0, &mxc_backlight_data_pwm2);
...
#ifdef CONFIG_MACH_MX53_NITROGEN_A
...
MACHINE_START(MX53_NITROGEN_A, "Boundary Devices Nitrogen_A MX53 Board")
...
#ifdef CONFIG_WL12XX_PLATFORM_DATA
static struct regulator_consumer_supply n53_vwl1271_consumers[] = {
	REGULATOR_SUPPLY("vmmc", "sdhci-esdhc-imx.2"),
};

static struct regulator_init_data n53_vwl1271_init = {
	.constraints            = {
		.name           = "VDD_1.8V",
		.valid_ops_mask = REGULATOR_CHANGE_STATUS,
	},
	.num_consumer_supplies = ARRAY_SIZE(n53_vwl1271_consumers),
	.consumer_supplies = n53_vwl1271_consumers,
};

static struct fixed_voltage_config n53_vwl1271_reg_config = {
	.supply_name		= "vwl1271",
	...
	.enable_high		= 1,
	.enabled_at_boot	= 0,
	.init_data		= &n53_vwl1271_init,
};

static struct platform_device n53_vwl1271_reg_devices = {
	.name	= "reg-fixed-voltage",
	.id	= 1,
	.dev	= {
		.platform_data = &n53_vwl1271_reg_config,
	},
};

struct wl12xx_platform_data n53_wlan_data __initdata = {
...
#ifdef CONFIG_WL12XX_PLATFORM_DATA
	/* WL12xx WLAN Init */
	if (wl12xx_set_platform_data(&n53_wlan_data))
		pr_err("error setting wl12xx data\n");
#if 1
	platform_device_register(&n53_vwl1271_reg_devices);
#else
...
MACHINE_START(MX53_NITROGEN, "Boundary Devices Nitrogen MX53 Board")
	.fixup = fixup_mxc_board,
	.map_io = mx53_map_io,
	.init_early = imx53_init_early,
...
#ifdef CONFIG_WL12XX_PLATFORM_DATA
static struct regulator_consumer_supply n53k_vwl1271_consumers[] = {
	REGULATOR_SUPPLY("vmmc", "sdhci-esdhc-imx.2"),
};

static struct regulator_init_data n53k_vwl1271_init = {
	.constraints            = {
		.name           = "VDD_1.8V",
		.valid_ops_mask = REGULATOR_CHANGE_STATUS,
	},
	.num_consumer_supplies = ARRAY_SIZE(n53k_vwl1271_consumers),
	.consumer_supplies = n53k_vwl1271_consumers,
};

static struct fixed_voltage_config n53k_vwl1271_reg_config = {
	.supply_name		= "vwl1271",
	.microvolts		= 1800000, /* 1.80V */
	...
	.enable_high		= 1,
	.enabled_at_boot	= 0,
	.init_data		= &n53k_vwl1271_init,
};

static struct platform_device n53k_vwl1271_reg_devices = {
	.name	= "reg-fixed-voltage",
	.id	= 1,
	.dev	= {
		.platform_data = &n53k_vwl1271_reg_config,
	},
};
...
static void __init n53k_board_init(void)
{
	...
	if (wl12xx_set_platform_data(&n53k_wlan_data))
		pr_err("error setting wl12xx data\n");
	platform_device_register(&n53k_vwl1271_reg_devices);
