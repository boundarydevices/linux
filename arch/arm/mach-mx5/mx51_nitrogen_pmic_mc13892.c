...
#define VSDEN_MASK		(1 << 18)
#define SD_STBY_MASK		(1 << 19)
#define VSDOP_MASK		(1 << 20)
...
#define REG_MODE_1_ALL_MASK	(GEN3_STBY_MASK | CAM_STBY_MASK |\
				...
				SD_STBY_MASK | VSDEN_MASK | VSDOP_MASK)
...
static int mc13892_regulator_init(struct mc13892 *mc13892)
{
	...
	stat = pmic_read_reg(REG_IDENTIFICATION, &value, 0xffffff);
	if (PMIC_SUCCESS != stat) {
		printk (KERN_ERR "%s: error reading pmic ID\n", __func__);
	} else
		printk (KERN_ERR "%s: PMIC id 0x%x\n", __func__, value );
...
static struct spi_board_info __initdata mc13892_spi_device = {
	...
	.max_speed_hz = 100000,	/* max spi SCK clock speed in HZ */
	.bus_num = 1,
	.chip_select = 0,
...
int __init mx51_nitrogen_init_mc13892(void)
{
